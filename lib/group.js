...
class Group {
   ...
   * @return {Array<Group>} return groups
   */
  async list({ login } = {}) {
   ...
   * @return {Group} return group info
   */
  async create({ data }) {
   ...
   * @param {Object} args - params
   * @param {Object} args.login - group login or id
   * @return {Array<User>} return group users
   */
   ...
   * @return {User} return group user info
   */
  ...
  async removeUser({ group, user }) {
    assert(group, 'group login or id is required');
    assert(user, 'user login or id is required');
    return this.client.request(`groups/${group}/users/${user}`, { method: 'DEELETE' });
  }
}
