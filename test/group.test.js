...
describe('test/group.test.js', () => {
  ...
  it.skip('should add and remove user', async () => {
    const data = await client.groups.addUser({ group: 'node-yuque', user: '', data: { role: 1 } });
    assert(data);

    const removeData = await client.groups.removeUser({ group: 'node-yuque', user: '' });
    assert(removeData);
  });
});
